...
import indexHtml1 from "@/components/arrival_air/index.html?raw";
import globalCss1 from "@/components/arrival_air/global.css?raw";
import styleCss1 from "@/components/arrival_air/style.css?raw";

// 🔴 Importaciones Hoja 2 (Términos/Condiciones o anexo)
import indexHtml2 from "@/components/arrival_air/2da hoja/index.html?raw";
import globalCss2 from "@/components/arrival_air/2da hoja/global.css?raw";
import styleCss2 from "@/components/arrival_air/2da hoja/style.css?raw";
...
function generateIframeContent(html: string, globalCss: string, styleCss: string, basePath: string) {
    ...
    <style>
      ...
      .arrival-notice, .arrival-notice-air {
        background-color: white; box-shadow: 0 0 40px rgba(0,0,0,0.1); margin: 0 auto 48px auto; overflow: hidden;
      }
    </style>
  `;
...
export function ArriboAir() {
    ...
    const fetchFolders = async () => {
        try {
            ...
            if (res.data && (res.data.status === "SUCCESS" || res.data.return === true)) setExistingFolders(res.data.data || []);
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        setFolderName(e.target.value); setSelectedFolderId(null); setShowSuggestions(true);
    };

    const handleSelectFolder = (folder: any) => {
        setFolderName(folder.nombre_carpeta); setSelectedFolderId(folder.id); setShowSuggestions(false);
    };

    const handleSave = async () => {
        if (!folderName.trim()) return toast.warning("El nombre de la carpeta es obligatorio");
        setIsGeneratingPdf(true);
        toast.info("Generando Arribo Air y guardando en el servidor...", { duration: 4000 });
        ...
        try {
            ...
            // Exportar Documento 1
            const pages1 = doc1.querySelectorAll(".arrival-notice");
            ...
            // Exportar Documento 2
            const pages2 = doc2.querySelectorAll(".arrival-notice-air");
            ...
            const fileName = `ArriboAir_${folderName.replace(/\s+/g, "_")}.pdf`;
            pdf.save(fileName);
            ...
            formData.append("url_documento_arribo_air", pdf.output("blob"), fileName);

            const res = await axios.post(`${API_URL}/guardardoc`, formData);
            if (res.data && (res.data.status === 200 || res.data.return === true)) {
                toast.success("¡Arribo Air guardado con éxito en la Nube!");
        ...
        } catch (error: any) { toast.error(`❌ ${error.message || "Ocurrió un error"}`); } finally { setIsGeneratingPdf(false); }
    };

    const filteredFolders = existingFolders.filter(f => f.nombre_carpeta?.toLowerCase().includes(folderName.toLowerCase()));

    const iframe1Src = generateIframeContent(indexHtml1, globalCss1, styleCss1, "/src/components/arrival_air/");
    const iframe2Src = generateIframeContent(indexHtml2, globalCss2, styleCss2, "/src/components/arrival_air/2da hoja/");
        ...
        <div className="min-h-screen bg-background flex flex-col overflow-hidden relative">
            ...
            <main className="flex-1 overflow-y-auto bg-slate-100/50 dark:bg-neutral-900 py-12 flex flex-col items-center gap-0 relative z-0">
                <div className="w-full max-w-[1000px] shrink-0 transition-all duration-300" style={{ height: `${iframe1Height}px` }}>
                    <iframe ref={iframe1Ref} srcDoc={iframe1Src} className="w-full h-full border-0 bg-transparent" title="Arribo Air - Pág 1" sandbox="allow-same-origin allow-scripts" />
                </div>
                <div className="w-full max-w-[1000px] shrink-0 transition-all duration-300" style={{ height: `${iframe2Height}px` }}>
                    <iframe ref={iframe2Ref} srcDoc={iframe2Src} className="w-full h-full border-0 bg-transparent" title="Arribo Air - Pág 2" sandbox="allow-same-origin allow-scripts" />
                </div>
            </main>
            ...
            {isSaveModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
                    <div className="bg-white rounded-xl shadow-2xl p-8 w-full max-w-md relative animate-in fade-in zoom-in duration-200">
                        <h2 className="text-2xl font-bold text-gray-800 mb-6">Guardar Arribo Air</h2>
