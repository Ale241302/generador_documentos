...
// 🔴 Importaciones apuntando a la carpeta invoice
import indexHtml from "@/components/invoice/index.html?raw";
import globalCss from "@/components/invoice/global.css?raw";
import styleCss from "@/components/invoice/style.css?raw";
...
function generateIframeContent(html: string, globalCss: string, styleCss: string) {
  ...
  `;
    ...
    const baseTag = `<base href="${window.location.origin}/src/components/invoice/" />`;
    return html.replace("</head>", `${baseTag}\n${styleTag}\n${visualFixes}\n</head>`);
}

// ─────────────────────────────────────────────
// COMPONENTE PRINCIPAL: Invoices
// ─────────────────────────────────────────────
export function Invoices() {
    const navigate = useNavigate();
    const iframeRef = useRef<HTMLIFrameElement>(null);

    const [iframeHeight, setIframeHeight] = useState(1529);
    ...
    const handleSave = async () => {
        ...
        toast.info("Generando Invoice y guardando en el servidor...", { duration: 4000 });
        ...
        try {
            ...
            tempStyle.innerHTML = `
                /* Aplicamos margen a los iconos específicos y a las imágenes que los contienen */
                .icon, 
                .icon-2, 
                .icon-3, 
            ...
            const pages = doc.querySelectorAll(".invoice");
            ...
            // 🔴 ELIMINAR EL ESTILO TEMPORAL PARA QUE LA VISTA WEB NO SE DAÑE 🔴
            const injectedStyle = doc.getElementById('pdf-export-icon-fix');
            if (injectedStyle) injectedStyle.remove();

            const fileName = `Invoice_${folderName.replace(/\s+/g, "_")}.pdf`;
            pdf.save(fileName);
            ...
            formData.append("url_documento_invoices", pdfBlob, fileName);

            const res = await axios.post(`${API_URL}/guardardoc`, formData);

            if (res.data && (res.data.status === 200 || res.data.return === true)) {
                toast.success("¡Invoice guardado con éxito en la Nube!");
                setIsSaveModalOpen(false);
                navigate("/dashboard");
            } else {
                throw new Error(res.data?.mensaje || res.data?.error || "Error al procesar la petición");
            }

        } catch (error: any) {
            console.error("Error en handleSave:", error);
            toast.error(`❌ ${error.message || "Ocurrió un error al generar o guardar"}`);
        } finally {
            ...
            {isSaveModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
                    <div className="bg-white rounded-xl shadow-2xl p-8 w-full max-w-md relative animate-in fade-in zoom-in duration-200">
                        <h2 className="text-2xl font-bold text-gray-800 mb-6">Guardar Invoice</h2>
                        ...
                        <div className="flex gap-4 justify-end mt-4">
                            <button
                                type="button"
                                className="px-6 py-2 border-2 border-[#f06e00] text-[#f06e00] rounded-lg font-bold hover:bg-[#f06e00] hover:text-white transition-colors disabled:opacity-50"
                            ...
                            <button
                                type="button"
                                className="px-6 py-2 bg-[#f06e00] text-white rounded-lg font-bold hover:bg-[#d05e00] transition-colors disabled:opacity-50"
                                onClick={handleSave}
                                disabled={isGeneratingPdf}
                            >
